...
export function buildSlotUserMessage({
  ...
  const userMessage =
    ...
    `24. **DERIVED VALUES CITE THEIR PARENTS (Sprint 65)** — Rule 21 forbids INVENTING numbers; correct arithmetic ON source numbers (growth rates, declines, shares, roundings) is allowed but must carry its parents inline, in parentheses, right where the value appears: "+580% (30.6 vs 4.5)", "-64% (46.0 vs 16.5)", "22.7% (1,483.2 vs 1,146.8)", "1.7M (1,696,180)". A reader must be able to recompute the value from the citation without leaving the slide. Never present a derived value bare, and only simple arithmetic of source numbers qualifies (difference %, ratio %, rounding) — anything requiring outside data remains forbidden by Rule 21.\n`;
