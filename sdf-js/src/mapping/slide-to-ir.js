...
- ARITY: nodes must have EXACTLY one label per value you emit. Reading 11 monthly bars → 11 labels and 11 values. Interpolating BETWEEN two printed axis dates is fine; if the axis prints NO time labels at all, you MUST use ①②… placeholders and set "axisUncertain": true — inventing a date range is the single worst audit failure (a deck dated Jan-2013 got a fabricated 2012.9–2014.2 axis this way). Never emit a time label later than the document's own date (title page / footer). Same for every series in a grouped chart.
...
- TWIN HISTOGRAMS: when a page stacks two similar bar charts, bind each chart's UNIT from its own axis annotation ((次) vs 分钟/秒) — swapping launch-count bins with session-duration bins inverts the page's claim.
