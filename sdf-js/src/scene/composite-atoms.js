  ...
  if (hasPostFx || hasCameraExt) {
    ...
    if (hasCameraExt) {
      ...
      for (const k of Object.keys(scene.defaults?.camera || {})) {
        if (scene.defaults.camera[k] !== undefined)
          out.defaults.camera[k] = scene.defaults.camera[k];
      }
    }
  }
  if (hasVolumes) {
...
function airportApronAtom(args = {}) {
  ...
  subjects.push({
    ...
    variants: [
      {
        op: 'array',
        count: parkedPlaneCount,
        axis: 'z',
        spacing: parkedPlaneSpacing,
        origin: 'center',
        scale: { jitter: 0.05 },
      },
    ],
...
function harborQuayAtom(args = {}) {
  ...
  if (withSea) {
    subjects.push({
      ...
      material: { hue: 0.58, sat: 0.6, value: 0.35, kind: 'sea' },
    });
  }
...
function concertStageAtom(args = {}) {
  ...
  if (withBackdrop) {
    subjects.push({
      ...
      material: { hue: 0.85, sat: 0.3, value: 0.2 },
    });
  }
  ...
  subjects.push({
    ...
    children: [
      ...
      },
      {
        ...
        material: { hue: 0.1, sat: 0.6, value: 1.0, glow: 0.9, kind: 'emissive' },
      },
    ],
    ...
    variants: [
      {
        op: 'array',
        count: stageLampCount,
        axis: 'x',
        spacing: stageLampSpacing,
        origin: 'center',
      },
    ],
  });
