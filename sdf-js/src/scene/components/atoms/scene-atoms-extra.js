...
import { sphere, box, cylinder, capsule, ellipsoid, cone, octahedron, pyramid, torus, tri_prism } from '../../../sdf/d3.js';
...
function _quadrupedSDF({
  ...
  const head = ellipsoid([headR * 1.2, headR * 0.9, headR * 0.85])
    .translate([bodyL * headXOff, headY, 0]);

  // 4 legs at body corners
  const lx = bodyL * 0.3, lz = bodyR * 0.65;
...
export function cowSDF({ scale = 1 } = {}) {
  return _quadrupedSDF({
    bodyL: 0.85 * scale, bodyR: 0.20 * scale,
    legH: 0.50 * scale, legR: 0.06 * scale,
    headR: 0.18 * scale,
    fillet: 0.02 * scale,
  });
}
...
export function horseSDF({ scale = 1 } = {}) {
  ...
  return _quadrupedSDF({
    bodyL: 0.95 * scale, bodyR: 0.17 * scale,
    legH: 0.65 * scale, legR: 0.05 * scale,
    headR: 0.13 * scale, headYBoost: 0.7,
    fillet: 0.018 * scale,
  });
}
...
export function pigSDF({ scale = 1 } = {}) {
  ...
  return _quadrupedSDF({
    bodyL: 0.65 * scale, bodyR: 0.22 * scale,
    legH: 0.18 * scale, legR: 0.05 * scale,
    headR: 0.13 * scale, headXOff: 0.45, headYBoost: 0.2,
    fillet: 0.025 * scale,
  });
}
...
export function dogSDF({ scale = 1 } = {}) {
  ...
  const body = _quadrupedSDF({
    bodyL: 0.50 * scale, bodyR: 0.10 * scale,
    legH: 0.25 * scale, legR: 0.035 * scale,
    headR: 0.09 * scale, headYBoost: 0.6,
    fillet: 0.012 * scale,
  });
  // Tail — angled-up capsule from rear of body
  const bodyY = 0.25 * scale + 0.10 * scale * 0.4;
  const tail = capsule(
    [-0.25 * scale, bodyY, 0],
    [-0.40 * scale, bodyY + 0.12 * scale, 0],
    0.025 * scale,
  );
...
export function sheepSDF({ scale = 1 } = {}) {
  ...
  const head = ellipsoid([0.10 * scale, 0.09 * scale, 0.08 * scale])
    .translate([bodyR * 0.95, bodyY - bodyR * 0.1, 0]);
  const lx = 0.13 * scale, lz = bodyR * 0.55;
  const mkLeg = (x, z) => capsule([x, bodyY - 0.05, z], [x, 0, z], 0.035 * scale);
  return unionRound(
    body, head,
    mkLeg(+lx, +lz), mkLeg(+lx, -lz),
    mkLeg(-lx, +lz), mkLeg(-lx, -lz),
    { r: 0.025 * scale },
  );
}
...
export function catSDF({ scale = 1 } = {}) {
  ...
  const body = _quadrupedSDF({
    bodyL: 0.40 * scale, bodyR: 0.08 * scale,
    legH: 0.20 * scale, legR: 0.025 * scale,
    headR: 0.08 * scale, headYBoost: 0.6,
    fillet: 0.010 * scale,
  });
  // Tail vertical-ish + slight curve (capsule pointing up)
  const bodyY = 0.20 * scale + 0.08 * scale * 0.4;
  const tail = capsule(
    [-0.20 * scale, bodyY, 0],
    [-0.18 * scale, bodyY + 0.22 * scale, 0.02 * scale],
    0.020 * scale,
  );
  return unionRound(body, tail, { r: 0.010 * scale });
}
...
export function rockBoulderSDF({ scale = 1 } = {}) {
  ...
  const b = ellipsoid([0.30 * scale, 0.28 * scale, 0.32 * scale])
    .translate([0.25 * scale, 0.12 * scale, 0.08 * scale]);
  const c = ellipsoid([0.25 * scale, 0.20 * scale, 0.26 * scale])
    .translate([-0.22 * scale, 0.05 * scale, -0.10 * scale]);
...
export function fenceSectionSDF({ length = 1.5, height = 0.5 } = {}) {
  ...
  const postR = 0.025, postH = height;
  const railR = 0.018;
...
export function streamSegmentSDF({ length = 2.0, width = 0.3, depth = 0.05 } = {}) {
  ...
  return capsule(
    [-length/2, 0, 0], [length/2, 0, 0], width / 2,
  ).scale([1, depth / (width / 2), 1]);
}
...
export function towerSquareSDF({ width = 1.0, height = 4.0, roofHeight = 0.8 } = {}) {
  ...
  const mid = box([width * 0.85, midH, width * 0.85])
    .translate([0, baseH + midH / 2, 0]);
  const roof = pyramid(roofHeight).scale(width * 0.9).translate([0, height, 0]);
...
export function churchSpireSDF({ width = 0.8, baseHeight = 1.5, spireHeight = 2.5 } = {}) {
  ...
  const spire = pyramid(spireHeight).scale(width * 0.85)
    .translate([0, baseHeight, 0]);
  // Small finial sphere at apex
  const finial = sphere(width * 0.04)
    .translate([0, baseHeight + spireHeight * 0.92, 0]);
...
export function fountainSDF({ radius = 0.7, basinHeight = 0.3 } = {}) {
  ...
  const pillar = cylinder(radius * 0.2, basinHeight * 1.6)
    .translate([0, basinHeight + basinHeight * 0.8, 0]);
  const upperBowl = cylinder(radius * 0.35, basinHeight * 0.3)
    .translate([0, basinHeight * 2.4, 0]);
...
export function sailboatSmallSDF({ scale = 1 } = {}) {
  ...
  const s = scale;
  const hull = capsule([-0.4 * s, 0, 0], [0.4 * s, 0, 0], 0.10 * s);
...
export function carSimpleSDF({ scale = 1 } = {}) {
  ...
  const wheelR = 0.08 * s, wheelT = 0.04 * s;
  const wheelCenter = 0.08 * s;
  const mkWheel = (xz) => torus(wheelR, wheelT)
    .translate([xz[0], wheelCenter, xz[1]])
    .rotate(Math.PI / 2, [1, 0, 0]);
  const wheels = [
    mkWheel([+0.30 * s, +0.15 * s]),
    mkWheel([+0.30 * s, -0.15 * s]),
    mkWheel([-0.30 * s, +0.15 * s]),
    mkWheel([-0.30 * s, -0.15 * s]),
  ];
...
export function wagonSDF({ scale = 1 } = {}) {
  ...
  const sideL = box([0.8 * s, 0.25 * s, 0.04 * s])
    .translate([0, 0.35 * s, +0.23 * s]);
  const sideR = box([0.8 * s, 0.25 * s, 0.04 * s])
    .translate([0, 0.35 * s, -0.23 * s]);
...
export function biplaneSDF({ scale = 1 } = {}) {
  ...
  // Upper wing
  const wingUpper = box([0.25 * s, 0.025 * s, 1.5 * s])
    .translate([0.05 * s, 0.58 * s, 0]);
  // Lower wing
  const wingLower = box([0.25 * s, 0.025 * s, 1.3 * s])
    .translate([0.05 * s, 0.30 * s, 0]);
  ...
  // Tail vertical fin
  const tailVert = box([0.15 * s, 0.18 * s, 0.02 * s])
    .translate([-0.55 * s, 0.50 * s, 0]);
  // Tail horizontal wing
  const tailHoriz = box([0.18 * s, 0.025 * s, 0.4 * s])
    .translate([-0.55 * s, 0.42 * s, 0]);
  // Propeller disc (flat torus at nose)
  const prop = torus(0.10 * s, 0.012 * s).translate([0.62 * s, 0.40 * s, 0]);
  return unionRound(
    fuselage, wingUpper, wingLower, strutL, strutR,
    tailVert, tailHoriz, prop, { r: 0.012 * s },
  );
}
...
export function chairSDF({ scale = 1 } = {}) {
  // 4 legs + seat + back
  const s = scale;
  const seatH = 0.45 * s, seatThick = 0.04 * s;
  const seat = box([0.4 * s, seatThick, 0.4 * s]).translate([0, seatH, 0]);
  const back = box([0.4 * s, 0.45 * s, 0.04 * s])
    .translate([0, seatH + 0.22 * s, -0.18 * s]);
  // 4 legs
  const lx = 0.17 * s, lz = 0.17 * s;
...
export function wineBottleSDF({ scale = 1 } = {}) {
  ...
  const s = scale;
  const body = cylinder(0.10 * s, 0.55 * s).translate([0, 0.275 * s, 0]);
  ...
  const neck = cylinder(0.035 * s, 0.20 * s).translate([0, 0.735 * s, 0]);
...
export function smokestackSDF({ radius = 0.25, height = 3.0 } = {}) {
  ...
  const cap = torus(radius * 1.10, radius * 0.08).translate([0, height, 0]);
...
export function windmillSDF({ scale = 1 } = {}) {
  ...
  // Hub
  const hub = sphere(0.08 * s).translate([0, 2.05 * s, 0.30 * s]);
  // 4 blades — long thin boxes at 90° intervals around hub Z-axis
  const bladeL = 1.5 * s, bladeW = 0.25 * s, bladeT = 0.04 * s;
  const blades = [];
  for (let i = 0; i < 4; i++) {
...
export function flowerSDF({ stemHeight = 0.6, bloomRadius = 0.12 } = {}) {
  ...
  const leafL = ellipsoid([0.05, 0.012, 0.025])
    .translate([+0.06, stemHeight * 0.5, 0]);
  const leafR = ellipsoid([0.05, 0.012, 0.025])
    .translate([-0.06, stemHeight * 0.4, 0]);
...
export function mushroomSDF({ stemHeight = 0.15, capRadius = 0.12 } = {}) {
  ...
  const cap = ellipsoid([capRadius, capRadius * 0.55, capRadius])
    .translate([0, stemHeight + capRadius * 0.45, 0]);
...
export function bushSDF({ radius = 0.4 } = {}) {
  ...
  const b = sphere(r * 0.70).translate([+r * 0.55, +r * 0.10, 0]);
  const c = sphere(r * 0.65).translate([-r * 0.55, +r * 0.05, +r * 0.10]);
...
export function vineSDF({ length = 1.0, thickness = 0.02 } = {}) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const lx = t * length;
    leaves.push(
      ellipsoid([0.06, 0.012, 0.025])
        .translate([lx, 0, side * 0.05]),
    );
  }
...
export function grassTuftSDF({ count = 5, height = 0.15 } = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    blades.push(
      capsule([dx, 0, dz], [dx * (1 + tilt), height, dz * (1 + tilt)], 0.008),
    );
  }
