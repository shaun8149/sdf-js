...
export function proceduralCitySDF({ blockSize = 1.0, maxHeight = 18.0, downtownK = 4.0 } = {}) {
  const inst = SDF3((p) => {
...
export const proceduralCitySpec = {
  ...
  args: {
    blockSize: {
      type: 'number',
      default: 1.0,
      doc: 'XZ tile size per block (1 world unit typical)',
    },
    maxHeight: { type: 'number', default: 18.0, doc: 'Peak skyscraper height in world units' },
    downtownK: {
      type: 'number',
      default: 4.0,
      doc: 'Center-bias strength (larger = more concentrated downtown)',
    },
  },
  source: {
    inspiration: 'Otavio Good Skyline (CC0 public domain Shadertoy)',
    algorithmRef:
      'per-block fract/floor SDF + hash randomization + downtown bias + window-floor quantization',
    license:
      'PolyForm Noncommercial 1.0.0 (Atlas reimplementation; source is CC0 so technically copy-allowed)',
    portedAt: '2026-05-25',
    porter: 'Atlas Sprint A7 — first procedural-scene primitive (city)',
    notes:
      'GPU-only. Requires u_cityActive uniform set by renderer for voxel-walk raymarch ' +
