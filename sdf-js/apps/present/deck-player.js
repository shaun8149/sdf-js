...
// from the shared scenes/ dir. Layer-2 sibling of present.js; calls only its
...
async function playDeck(id) {
  ...
  async function go(index) {
    ...
    try {
      await window.present.load(seg.file);
  ...
  // On-screen touch nav (keyboard ← → Space P R still work).
  const navPrev = document.getElementById('nav-prev');
  const navNext = document.getElementById('nav-next');
  if (navPrev) navPrev.addEventListener('click', () => go(cur - 1));
  if (navNext) navNext.addEventListener('click', () => go(cur + 1));
