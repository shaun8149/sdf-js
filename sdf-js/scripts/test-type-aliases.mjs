...
console.log(`Registry has ${PRIMITIVE_TYPES.size} types; built ${Object.keys(PRIMITIVE_TYPE_ALIASES).length} aliases.`);
...
ok(normalizeType('rounded_cylinder')=== 'rounded-cylinder','snake alias (v3.10 dining-setting bug) → fixed');
...
for (const [alias, canon] of Object.entries(PRIMITIVE_TYPE_ALIASES)) {
  if (PRIMITIVE_TYPES.has(alias)) { collisions++; console.log(`  collision: ${alias} is both canonical AND aliased to ${canon}`); }
}
...
console.log('\n[e2e] LLM-style snake_case emit compiles via alias:');
{
  ...
  if (compiled) {
    ok(scene.subjects[0].type === 'capped-torus',
       `subject 0 type mutated to canonical (got "${scene.subjects[0].type}")`);
    ok(scene.subjects[1].type === 'rounded-cylinder',
