...
console.log('\n[scatter] Phase 1 regression:');
{
  ...
  ok(out.subjects.every(s => s.id.startsWith('tree-')), 'ids suffix tree-0..4');
  ok(out.subjects.every(s => !s.variants), 'variants field stripped');
  ok(out.subjects.every(s => Array.isArray(s.transform.translate)), 'each has translate');
}
...
console.log('\n[array] equispaced along x, origin=center:');
{
  const scene = {
    subjects: [{
      id: 'pillar',
      type: 'box', args: { dims: [0.4, 3, 0.4] },
      transform: { translate: [0, 1.5, 0] },
      variants: [{
        op: 'array', count: 5, axis: 'x', spacing: 2.0, origin: 'center',
      }],
    }],
  };
  const out = expandVariants(scene, new Random(HASH));
  ...
  ok(out.subjects.every(s => Math.abs(s.transform.translate[1] - 1.5) < 1e-9), 'y preserved at 1.5');
  // Z unchanged
  ok(out.subjects.every(s => s.transform.translate[2] === 0), 'z preserved at 0');
}
...
console.log('\n[array] origin=start, count=3, axis=z:');
{
  const scene = {
    subjects: [{
      id: 'step', type: 'box', args: { dims: [1, 0.2, 1] },
      transform: { translate: [0, 0, 0] },
  ...
  ok(zs[0] === 0 && zs[1] === 1 && zs[2] === 2, `start mode zs=[0,1,2] (got ${JSON.stringify(zs)})`);
}
...
console.log('\n[array] custom axis vector + scale jitter:');
{
  const scene = {
    subjects: [{
      id: 'beam',
      type: 'sphere', args: { radius: 0.3 },
      transform: { translate: [0, 0, 0], scale: 1.0 },
      variants: [{
        op: 'array', count: 4, axis: [1, 1, 0], spacing: 1.414,
        scale: { jitter: 0.1 },
      }],
    }],
  };
  ...
  ok(out.subjects.every(s => typeof s.transform.scale === 'number'), 'scale is number per copy');
  ok(out.subjects.every(s => s.transform.scale > 0.85 && s.transform.scale < 1.15), 'scale within jitter band');
}
...
console.log('\n[mirror] yz plane (default), flip X:');
{
  const scene = {
    subjects: [{
      id: 'wing', type: 'box', args: { dims: [2, 0.1, 0.5] },
...
console.log('\n[mirror] phaseFlip on legacy flat-phase form (back-compat):');
{
  const scene = {
    subjects: [{
      id: 'legacy', type: 'box', args: { dims: [1, 0.1, 0.3] },
      transform: { translate: [1, 0, 0] },
      animation: [{ phase: 0 }],
      variants: [{ op: 'mirror', plane: 'yz', phaseFlip: Math.PI }],
    }],
...
console.log('\n[mirror] xz plane flips Y:');
{
  ...
  const out = expandVariants(scene, new Random(HASH));
  ok(out.subjects[1].transform.translate[1] === -2, `xz mirror y=-2 (got ${out.subjects[1].transform.translate[1]})`);
}
...
console.log('\n[id rewrite] children get -${i} suffix:');
{
  ...
  ok(out.subjects[0].children[0].id === 'piece-a-0', `child piece-a-0 (got ${out.subjects[0].children[0].id})`);
  ok(out.subjects[2].children[1].id === 'piece-b-2', `child piece-b-2 (got ${out.subjects[2].children[1].id})`);
}
...
console.log('\n[determinism] same hash → same expansion:');
{
  const sceneTpl = () => ({
    subjects: [{
      id: 'bird', type: 'sphere', args: { radius: 0.2 },
  ...
  const sameXs = a.subjects.every((s, i) =>
    s.transform.translate[0] === b.subjects[i].transform.translate[0]);
...
{
  const scene = {
    subjects: [{ id: 'a', type: 'sphere', args: { radius: 1 }, transform: { translate: [0,0,0] } }],
  };
  const out = expandVariants(scene, new Random(HASH));
