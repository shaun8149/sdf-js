...
ok(
  typeof staged.defaults.interiorDark === 'number' && staged.defaults.interiorDark < 1,
  'interiorDark set (< 1)',
);
...
ok(
  lights.some((L) => Array.isArray(L.dir)),
  'key light is a spotlight (has dir)',
);
...
ok(
  staged.cameraSequence.shots.every((s) => s.aperture > 0 && s.focalDistance > 0),
  'every shot has aperture + focalDistance',
);
{
  ...
  ok(
    Math.abs(s2.cameraSequence.shots[0].aperture - 0.42) < 1e-9,
    'a shot that already sets aperture keeps it',
  );
}

// immutability: the input scene is not mutated
ok(
  base.subjects.every((s) => s.id !== 'stage-platform'),
  'original scene not mutated (no platform)',
);
...
ok(
  viaOpts.subjects.some((s) => s.id === 'stage-platform'),
  'renderIR(ir, {stage:true}) applies the preset',
);
...
// platform fits the structure: a wide magnitude row gets a bigger disc than a funnel
{
  ...
  const inside = wide.subjects
    ...
    .every((s) => {
      ...
      return (
        Math.hypot(tr[0] - pWide.transform.translate[0], tr[2] - pWide.transform.translate[2]) <=
        pWide.args.radius
      );
    });
...
// deck: assembleDeck({stage:true}) → per-station platforms + deck theatre defaults
{
  ...
  ok(
    platforms[0].transform.translate[0] !== platforms[1].transform.translate[0],
    'platforms sit at their stations',
  );
  ok(staged.defaults.glow && staged.defaults.glow.amount > 0, 'deck theatre: glow on');
  ok(
    staged.defaults.postFx && staged.defaults.postFx.vignetteStrength > 0,
    'deck theatre: vignette on',
  );
  const plain = assembleDeck(deck, {});
  ok(
    !plain.subjects.some((s) => s.id.includes('stage-platform')),
    'deck without stage unchanged (no platforms)',
  );
