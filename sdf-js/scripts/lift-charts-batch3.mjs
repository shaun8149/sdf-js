...
for (const t of TEMPLATES) {
  ...
  writeFileSync(
    resolve(SCENES, `lifted-charts-${t.name}.json`),
    `${JSON.stringify(scene, null, 2)}\n`,
  );
  console.log(
    `  ✓ ${t.subjects[0].type.padEnd(18)} (2D data) → ${scene.subjects[0].type.padEnd(20)} → scenes/lifted-charts-${t.name}.json`,
  );
  lifted.push(t.name);
}
