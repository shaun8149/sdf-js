...
const tests = [
  ...
  },
  {
    ...
    scene: {
      ...
      defaults: {
        ...baseScene.defaults,
        camera: { yaw: 0, pitch: 0, distance: 0.5, focal: 1.0, targetX: 0, targetY: 0, targetZ: 0 },
      },
...
const whitelistNegativeTests = [
  {
    ...
    scene: {
      ...baseScene,
      subjects: [{ id: 'runway-strip', type: 'box', args: { dims: [80, 0.1, 4] } }],
    },
  },
  {
    ...
    scene: {
      ...baseScene,
      subjects: [{ id: 'apron-ground', type: 'box', args: { dims: [120, 0.05, 80] } }],
    },
  },
  {
    ...
    scene: {
      ...baseScene,
      subjects: [{ id: 'canyon-hero', type: 'terrain-canyon', args: { depth: 80 } }],
    },
  },
  {
    ...
    scene: {
      ...baseScene,
      subjects: [{ id: 'runway-1', type: 'box', args: { dims: [500, 1, 4] } }],
    },
    expect: 'large-arg',
  },
  {
    name: 'non-whitelisted subject still warned at 80 (sphere radius=80)',
    scene: { ...baseScene, subjects: [{ id: 'random', type: 'sphere', args: { radius: 80 } }] },
    expect: 'large-arg',
  },
];
...
for (const t of whitelistNegativeTests) {
  ...
  if (ok) {
    ...
    console.log(`✗ ${t.name} — got: ${r.summary}`);
    wlFail++;
  }
}
