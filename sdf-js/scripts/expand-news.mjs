#!/usr/bin/env node
// =============================================================================
// expand-news.mjs — Sprint 30 Stage -1: raw news text → slidedata JSON
// -----------------------------------------------------------------------------
// The missing front door of the heavy pipeline: bake-scaffold-pipeline.mjs
// eats pre-split slidedata; real users have a pasted ~500-char news article.
// This stage EXPANDS prose into a 12-18 slide outline (target band 10-20
// pages after mapping loss), one fact-cluster per slide, all numbers and
// named entities carried verbatim (Rule 18/20 upstream: the lift can only
// preserve what the outline preserved).
//
// Deterministic floors (stability > prompt persuasion, per Sprint 25 lesson):
//   - < MIN_SLIDES after the LLM call → one retry with an explicit count
//     complaint; still short → mechanical split (longest slides' body lines
//     become their own slides) until the floor is met.
//   - > MAX_SLIDES → tail slides merged pairwise until under the ceiling.
//
// Usage:
//   node sdf-js/scripts/expand-news.mjs --text article.txt --out slides.json \
//     [--key-file key.txt] [--min 12] [--max 18]
//
// Also exported as expandNews(text, {apiKey, min, max}) for news-to-deck.mjs.
// =============================================================================
