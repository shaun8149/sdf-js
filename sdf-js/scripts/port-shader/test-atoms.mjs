...
const TESTS = [
  ...
  ['moon', { radius: 0.4 }, [0, 0, 0], [10, 0, 0], ['sdSphere']],
  ['star', { radius: 0.1, shape: 'octahedron' }, [0, 0, 0], [1, 1, 1], ['sdOctahedron']],
  ['sun', { radius: 0.4, haloThickness: 0.06 }, [0, 0, 0], [10, 0, 0], ['sdSphere']],
  ['cloud-puff', { width: 1, height: 0.45, depth: 0.6 }, [0, 0, 0], [10, 10, 10], ['sdEllipsoid']],
  [
    'tree-pine',
...
for (const [type, args, inside, outside, expectedPrims] of TESTS) {
  ...
  try {
    ...
    fail++;
    console.log(`  ✗ ${type}: compile error`);
    continue;
  }
  ...
  if (!insideOK) {
    failures.push(`${type}: inside probe ${inside} returned ${dIn.toFixed(4)}`);
    allOK = false;
  }
  if (!outsideOK) {
    failures.push(`${type}: outside probe ${outside} returned ${dOut.toFixed(4)}`);
    allOK = false;
  }
  ...
  if (glsl.error) {
    ...
    for (const prim of expectedPrims) {
      ...
      if (!hit) {
        failures.push(`${type}: GLSL missing ${prim}`);
        allOK = false;
      }
    }
  }
