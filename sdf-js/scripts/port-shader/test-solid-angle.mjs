...
console.log(
  'Inside the cone (on +Y axis, near tip)        :',
  cone.f([0, 0.1, 0]).toFixed(4),
  '(expect < 0)',
);
console.log(
  'Inside the cone (near outer spherical cap)    :',
  cone.f([0, 0.4, 0]).toFixed(4),
  '(expect < 0)',
);
console.log(
  'Just outside aperture (off-axis)              :',
  cone.f([0.3, 0.2, 0]).toFixed(4),
  '(expect > 0)',
);
console.log(
  'Far above on axis (past spherical cap)        :',
  cone.f([0, 2.0, 0]).toFixed(4),
  '(expect > 0)',
);
console.log(
  'Below the tip                                 :',
  cone.f([0, -0.5, 0]).toFixed(4),
  '(expect > 0)',
);
console.log(
  'Far side                                      :',
  cone.f([2, 0, 0]).toFixed(4),
  '(expect > 0)',
);
console.log(
  'At the tip (origin)                           :',
  cone.f([0, 0, 0]).toFixed(4),
  '(expect = 0 or ≈ 0)',
);
...
for (const deg of [10, 20, 30, 45, 60]) {
  ...
  console.log(
    `  ${deg}°: on-axis(0.3) = ${onAxis.toFixed(3)} | off-axis(0.3,0.3) = ${offAxis.toFixed(3)}`,
  );
}
