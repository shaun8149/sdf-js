...
for (const [type, args, probes, glslSub] of TESTS) {
  ...
  catch (e) { failures.push(`${type}: compile() failed: ${e.message}`); fail++; continue; }
  ...
  for (const [label, pt, expectedSign] of probes) {
    ...
    const tag = ok ? '✓' : '✗';
    console.log(`  ${tag} ${type} :: ${label} = ${d.toFixed(4)} (expect ${expectedSign > 0 ? '>0' : '<0'})`);
    if (!ok) { allProbesOK = false; failures.push(`${type} :: ${label} returned ${d.toFixed(4)}, expected sign ${expectedSign}`); }
  }
  ...
  if (!hasCall) {
    failures.push(`${type}: GLSL output missing "${glslSub}". Error: ${glsl.error ?? '(no error)'}`);
    allProbesOK = false;
  }
