...
console.log('On tube center, x=R, y=0 (inside tube core)   :', link.f([0.1,  0,    0]).toFixed(4), '(expect ≈ -0.02)');
// On the tube outer surface
console.log('On tube outer surface, x=R+r, y=0             :', link.f([0.12, 0,    0]).toFixed(4), '(expect ≈ 0)');
// In the hole of the link (between the two parallel sections)
console.log('In the hole, x=0, y=0 (axis center)           :', link.f([0,    0,    0]).toFixed(4), '(expect > 0, ≈ R - r = 0.08)');
// Inside the elongated portion (|y| < le)
console.log('Inside elongated section, x=R, y=0.1          :', link.f([0.1,  0.1,  0]).toFixed(4), '(expect ≈ -0.02, same as y=0)');
// Just past the end cap
console.log('Outside, y past end-cap                       :', link.f([0,    0.5,  0]).toFixed(4), '(expect > 0)');
// On Z-axis side
console.log('Outside, z direction at tube center           :', link.f([0,    0,    0.12]).toFixed(4), '(expect ≈ 0)');
// Far away
console.log('Far away                                      :', link.f([2,    2,    2]).toFixed(4), '(expect > 0, large)');

console.log('\n═══ ASCII silhouette — XY plane, orthographic +Z view ═══');
// Render an XY slice (Z=0) at higher resolution to see the link's loop shape
const W = 60, H = 30;
const viewX = 0.25, viewY = 0.4;
let plot = '';
...
for (const le of [0.0, 0.1, 0.2, 0.4]) {
  ...
  console.log(`  le=${le.toFixed(2)}: hole(0,0,0)=${onAxis.toFixed(3)} | tube(R,0,0)=${onTube.toFixed(3)} | far-y=${farY.toFixed(3)}`);
}
...
for (const r1 of [0.05, 0.1, 0.15, 0.2]) {
  ...
  console.log(`  r1=${r1.toFixed(2)}: tube(r1,0,0)=${onTube.toFixed(3)} | hole(0,0,0)=${hole.toFixed(3)}`);
}
