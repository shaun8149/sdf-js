...
const scene = {
  ...
  id: 'fighter-jet',
  defaults: {
    camera: { yaw: 0.5, pitch: 0.3, distance: 10, focal: 1.8, targetX: 0, targetY: 0, targetZ: 0 },
  ...
  subjects: [
    // Fuselage
    {
      id: 'fuselage',
      type: 'capsule',
      args: { a: [0, 0, -2.5], b: [0, 0, 2.5], radius: 0.4 },
...
if (expanded.subjects.length !== 7) {
  console.error(
    `✗ expected 7 subjects (1 fuselage + 4 fins + 2 wings), got ${expanded.subjects.length}`,
  );
  process.exit(1);
}
...
if (dCenter > 0) {
  console.error('✗ center should be inside fuselage');
  process.exit(1);
}
if (dFar < 0) {
  console.error('✗ far should be outside');
  process.exit(1);
}
...
console.log(
  `✓ wing anim phase: left=${leftPhase}, right=${rightPhase} (Δ = ${(rightPhase - leftPhase).toFixed(4)})`,
);
