...
**Other ops planned for later phases** (don't emit yet, no implementation):
`array` (line / grid, deterministic spacing — for column rows, palisades),
`mirror` (generator-level reflection — for double towers, twin-mast ships,
butterflies), `augment` (LLM-driven "carrier → strike group" semantic
enrichment).
