...
ok(
  plain.length === 1 && Math.abs(plain[0].t - 5) < 1e-9 && plain[0].kind === 'finale',
  'untagged → single full-span beat',
);
...
ok(
  db.some((b) => b.kind === 'super'),
  'deck has a super beat (emphasis)',
);
ok(db[db.length - 1].kind === 'finale', 'deck ends on the finale beat');
ok(
  db.every((b, i) => i === 0 || b.t > db[i - 1].t),
