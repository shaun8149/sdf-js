...
console.log('\n[pentagon] radius=0.5 (apothem)');
{
  ...
  ok(f(0, 0.618) > -0.02 && f(0, 0.618) < 0.02, `top vertex (0, 0.618) ≈ on edge (got ${f(0,0.618).toFixed(3)})`);
...
console.log('\n[hexagram] radius=0.3 (outer tip ≈ 0.6)');
{
  ...
  const f = (x, y) => sdf.f([x, y]);
  ok(f(0, 0) < 0, `center inside (got ${f(0,0).toFixed(3)})`);
  ok(f(0, 0.6) > -0.05 && f(0, 0.6) < 0.05, `top outer tip ≈ on edge (got ${f(0,0.6).toFixed(3)})`);
...
console.log('\n[e2e] All 5 types compile via SceneData:');
{
  for (const [type, args] of [
    ...
    const scene = {
      ...
      subjects: [{ id: `p-${type}`, type, args,
                   transform: { translate: [0, 0, 0] } }],
    };
    let threw = null;
    try { compile(scene); } catch (e) { threw = e.message; }
