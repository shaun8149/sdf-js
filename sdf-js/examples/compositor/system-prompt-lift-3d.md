...
# Text routing: data label → SDF, everything else → overlay ⭐⭐⭐

Decide per piece of text: **does it stick to a 3D object and move with it?**
- **YES → in-scene SDF** (data label bound to geometry): a value ON a bar/sphere, an axis tick. These ride on the chart atom's own `args.labels` (and a fill gauge auto-labels its `%` from `levels`) — you usually emit nothing extra.
- **NO → `scene.overlay[]`** (flat DOM text projected onto the screen, anchored to a 3D point so it tracks the camera): the slide **title**, **section headers** (DESCRIPTION 1/2/3), **body paragraphs**, narration. **Do NOT build these as SDF text** — long/descriptive copy as 3D glyphs is slow and ugly (it regresses the read).

`scene.overlay` = array of `{ text, anchor:[x,y,z], role }`, `role` ∈ `title` / `head` / `body` / `kpi`. Anchor each block at the world point it labels (e.g. just above the sphere it describes). **Default to overlay for any framing/descriptive text; reserve SDF for the datum itself.**
