...
version: 3.38
description: Take an existing sdf-js 2D scene (user prompt + generated SDF code) and lift it into a 3D world the user can fly through. Output Atlas SceneData v1 JSON with 3D primitives, camera, light, ground, and shadow — render-ready by `compile()` + BOB GPU shader. Trigger after user clicks "✨ Lift to 3D" on a 2D scene they liked. v2.1 added material/pattern presets + hg_sdf boolean variants + facade-to-3D mass synthesis. v2.2 added 5 dining presets. v2.3 added decision heuristic + bicycle Example 5. v3.0 expanded atom library 9 → 42 (animals/landscape/architecture/vehicles/furniture/mechanical/plants). v3.1 adds MANDATORY scene contextual augmentation. v3.4 ships 8 new IQ canonical 3D primitives (cut-sphere, cut-hollow-sphere, death-star, rounded-cylinder, round-cone-ab, vesica-segment, cylinder-inf, cone-inf), 3 new ops (xor, displace, elongate-correct), 6 smooth-min variants (unionExp/Cubic/Quartic/Circular/CircGeo/Root), AND fixes revolve + extrude on the GPU side. v3.5 (2026-05-23) adds TWO worked examples to drive adoption: Example 8 (fruit bowl — uses cut-hollow-sphere, vesica-segment, round-cone-ab, death-star in one scene) and Example 9 (Generator-S `variants[]` scatter spec for 1 prototype → N instances at zero token cost). v3.4 LLM ignored the new primitives because there was no example; v3.5 reverses that. v3.7 (2026-05-24) unlocks the CINEMATIC AXIS — Sprint 1-6 capabilities now LLM-emittable: `defaults.postFx` (HDR bloom + DoF + lens flare + ACES tonemap), top-level `volumes[]` (smoke / flame / fog / god-rays), top-level `cameraSequence` (multi-shot timeline with sceneState ramps + shake), `cameraSequence.subjectMotion` (CarInt physics — subjects fly), `volume.attachTo` + `sceneStateKey` (exhaust follows rocket, density modulated by shot phase), `shot.pos.relativeTo` + `target.relativeTo` (camera tracks moving subjects), `shake.velocityScale` (camera shake scales with subject velocity). Worked Examples 10-13 cover each cluster — STUDY THEM before emitting any cinematic scene. v3.8 (2026-05-25) fixes two issues caught by v3.5-vs-v3.7 regression: (1) DomainGroup args spec was ambiguous (`period? | axis? | k?` lined with `|` made LLM mix fields across types — caused `vintage-bicycle` v3.7 compile fail) — now split into per-type table with explicit Wrong→Right; (2) `modPolar` / `mirrorOctant` / `curve` / `elongate` / `displace` were missing from the DomainGroup table entirely — added with use-cases. Plus new Worked Example 14 (wheel spokes / flower petals / fan blades via `modPolar`). v3.9 (2026-05-26) fixes two issues caught by v3.5-vs-v3.7 regression: (1) `brass` / `leather` material preset hallucination (vintage-bicycle v3.8 fired 8 unknown-preset warnings) — added explicit 20-name whitelist with ❌ trap section + 2 missing presets (`deep-water` / `shallow-water`) added to visible list; (2) weak-cue cinematic adoption — added Example 11b teaching the LLM that prompts naming a *mood* (night / dusk / 雾 / 雪 / 废墟) or *motion subject* (流星 / 火箭 / 战机) deserve at minimum `postFx + 1 ambient volume + DoF aperture`, even without explicit cinematic trigger words. Counter-examples included so single-object scenes (花瓶 / 自行车 / 9:15 钟) don't over-trigger. v3.10 (2026-05-26) ships Generator-S Phase 2 ops — `array` (deterministic equispaced copies with per-instance jitter) and `mirror` (bilateral plane reflection with optional anim phase flip). Adds Example 9b (colonnade via `array`) + Example 9c (fighter-jet wings via `mirror` with `phaseFlip`) + 5 counter-examples and a Generator-S decision cheatsheet. Replaces the v3.9 "other ops planned for later phases" placeholder. v3.11 (2026-05-26) introduces SCENE COMPLETION — singular noun prompts ("航母" / "机场塔台" / "港口") trigger PEER-LEVEL world generation, not just foreground decoration (which is v3.1's job). When prompt = lone military or transit hub, emit the implied surrounding peer subjects via Generator-S Phase 2 ops: carrier → escort fleet (`scatter`), airport → parked planes (`array`), highway → cars (`array`). Distinct from v3.1 contextual augmentation (foreground decorations like trees-on-mountain): v3.11 completes the peer world. 6-row trigger table + Worked Example 15 (lone-carrier) + 4 counter-examples (single-object focused prompts like 教堂/瓶子/钟/F-22 双翼 stay singular). v3.12 (2026-05-26) fixes cross-axis tradeoff caught by v3.10-vs-v3.11 regression: lone-carrier in v3.11 dropped cinematic budget (`cin[pfx1/ap1/vol1]` → `cin[pfx0/ap0/vol0]`) — LLM treated scene completion + cinematic as mutually exclusive. v3.12 adds explicit "both apply simultaneously" directive in Scene completion section + extends Example 15 with cinematic block (defaults.postFx + camera.aperture + 1 fog volume) so byte-precision teaching includes cinematic alongside peer subjects. v3.13 (2026-05-26) ships 3 high-level COMPOSITE ATOMS (`carrier-strike-group` / `airport-apron` / `harbor-quay`) — single-line emit shortcuts that compile-time-expand into the full peer-subject constellation + cinematic patches. Distinct from v3.11 hand-emit path (which stays valid for non-standard scenes). Adds Worked Example 16 (lone-carrier via composite atom — 1 line vs Example 15's ~25 lines) + cheatsheet decision row. v3.14 (2026-05-27) extends Scene completion to LEISURE (concert / stadium / market) + NATURE (canyon / coastal) categories. 5 new trigger rows added to Scene-completion table covering crowd-aggregation scenes (concert audience + stage lights / stadium tiered seats / market stalls) and geographic landscape scenes (canyon river + ridge + birds / coastal rocks + waves + lighthouse). Adds Worked Example 17 (lone "音乐会" concert scene completion via scatter audience + array stage lights). v3.11/v3.12/v3.13 paths unchanged. v3.15 (2026-05-27) fixes composite atom adoption regression caught in v3.14 cross-axis 18-demo matrix: atom emit dropped from 4/4 (v3.13) to 1/4 (v3.14) because Example 17's hand-emit pattern diluted Example 16's atom shortcut signal. v3.15 adds explicit cross-reference from Example 17 → Example 16, hardens the decision row with PRIORITY language (composite atom is PREFERRED for canonical military/transit prompts), and adds atom usage reminder in scene-completion section intro. No code changes — pure prompt re-balancing. v3.16 (2026-06-19) introduces the text-3d atom dichotomy: `text-3d-extruded` (renamed from `text-3d`, 2D outline pushed along Z — for surface-flush signage, axis labels, watermarks) and `text-3d-pipe` (NEW, true 3D from capsules + toruses + spheres — for floating monumental text, KPI heroes, sculpted slide titles). Decision heuristic in prompt: \"floating-in-space → pipe; stuck-to-surface → extruded.\" Worked Examples 18a (extruded axis label flat on floor) + 18b (pipe hero KPI floating). 4 traps documented (under-thin pipe, over-deep extruded, pipe lying flat, wall text using pipe). v3.17 (2026-06-19) — Wave 2 typography ships 26 uppercase letters A-Z in both text-3d-pipe + text-3d-extruded atoms. v3.16's \"Letters drop silently — fix by waiting for Wave 2/3\" caveat removed. Slide titles like \"FILL LEVELS\" / \"VISION 2030\" / \"3D SPHERES\" now render as actual SDF text instead of placeholder rectangles. Lowercase + punctuation still pending Wave 3-4. v3.18 (2026-06-20) adds 3-step archetype-first discipline (Steps 1-3) — covers all 5 antvis-validated dimensions: (1) first-line constraint via MANDATORY scene.name `"<archetype>: <title>"`; (2) 7-class taxonomy (sequence/list/compare/hierarchy/relation/kpi-hero/text-card; chart→kpi-hero + quadrant folded into compare + new text-card fallback); (3) data-field constraint via Step 2 stick-to-archetype consistency rule (no mixing archetypes top-level); (4) decision algorithm via Step 1 "When to pick" column; (5) icon discipline via Step 3 semantic marker rule (every list item/sequence step/hierarchy node carries a marker — text-3d-pipe digit/glyph or small primitive — to prevent unlabeled-blob failure mode). Inspired by AntV/Infographic + LangChat Slides production evidence. Slide-archetype is now the LLM's FIRST decision before emitting subjects. Fixes "free-form scene on text-heavy slide" failure mode (Aether AI pages 1/6/7/9/11 black-blob class) + the "abstract unlabeled identical-primitive blob" failure mode. v3.19 (2026-06-20) flags 2D-mode constraints: callLiftLLM(opts.mode='2d') appends an addendum forbidding text-3d-extruded / text-3d-pipe subjects (Atlas Present 2D renderers — silhouette/lines/crayon/topo — cannot render SDF glyphs cleanly; text in 2D mode renders via Canvas2D outside the SDF tree). Runtime sanitize2dSceneData() is defense layer 2. Sprint 1.5 lesson: variant generation on text-heavy paragraphs produced ugly SDF text glyphs (per manual L3 reject 2026-06-20). v3.20 (2026-06-20 afternoon) extends MODE_2D_ADDENDUM (in src/compositor-api.js) with Step 4 — 3-priority content-driven routing for 2D mode: Priority 1 (SDF DIRECT, literal concrete; Sprint 1 v4 capability), Priority 2 (SDF METAPHORICAL, concrete composition for abstract — carrier-from-coins pattern, Atlas's real wedge that vector cannot replicate), Priority 3 (P5 FALLBACK, graceful degradation via single p5-sketch subject). 6-variant allocation: ~3-4 SDF wedge + ~2-3 P5 fallback. Solves Sprint 1.5 variant convergence on text-heavy content. p5-sketch is new subject type carrying P5 sketch in args.code; runs in iframe sandbox with 28 Atlas SDF helpers + branding palette. Constraint: single-subject only (no mixing p5-sketch with traditional types in Sprint 3; Sprint 4+ may relax). 3 worked examples embedded in addendum (Priority 1 cathedral, Priority 2 ⭐ \$3B carrier-from-coins, Priority 3 sequence-of-boxes). v3.21 (2026-06-20 evening, Sprint 4 — P5 idiom registry from Gorilla Sun port) introduces 5 higher-level P5 patterns LLM can inline into args.code: (1) packCirclesInSDF — variable-size organic packing of any SDF outline (upgrades v3.20 uniform-grid coin fill to dense organic carrier-of-coins, sourced from Ahmad Moussa recursive circle packing article); (2) irregularGridPack — multi-pass boolean-grid non-uniform KPI card layout, fixes L6-6C uniform-grid SaaS metric regression (1 hero + 2 medium + 4 small cards instead of 8 identical boxes); (3) packShapes — spatial-hash grid collision for filling SDF region with arbitrary shapes (generalizes wedge from coins-only to dollar-bills/soldiers/icons); (4) createGraphics buffer + image() stamp — render expensive sub-pattern once stamp N times (3 carriers L6-6A performance); (5) roundedPolyPath — any-shape rounded corners via canvas arcTo (hierarchy/network node aesthetics). All sourced from Gorilla Sun (Ahmad Moussa) gorillasun.de articles as recipe-only ports — patterns inlined into args.code by LLM, NOT auto-loaded into iframe helper bundle. 2 new worked examples in addendum: Priority 2 upgrade (carrier with 600 variable-size coins via packCirclesInSDF) + Priority 2/3 hybrid (irregular KPI dashboard via irregularGridPack with $100M ARR hero + smaller metrics). v3.22 (2026-06-20 late evening, Sprint 5 — Gorilla Sun Tier B absorption) extends idiom registry with 3 more patterns: (6) delaunayTriangles + voronoiCells — Bowyer-Watson incremental Delaunay + circumcenter-dual Voronoi for organic region partitioning (market share / population density territory visualization where rectangular grid feels wrong); (7) buildFlowField + traceFlowLines — Perlin-noise-derived 2D angle grid + traced flow polylines for SUBTLE background texture giving Atlas P5 output distinctive visual identity vs flat Napkin/antvis (deterministic 2D value noise replaces P5's noise() so output is reproducible); (8) springBrushStroke — Hooke-spring-physics chain walked along input waypoints for hand-drawn line aesthetic with thickness modulated by local speed (kills "AI sterile vector" feel on connector lines / arrows / annotations; Moussa credits BUN upstream). 2 NEW worked examples added: "Cloud Market Share Territories" (Voronoi cells with opacity proportional to share — AWS 32% biggest darkest, GCP 11% smallest faintest) + "Architecture Flow Hand-Drawn" (Backend → Frontend → Mobile boxes connected by wobbly Hooke-spring arrows instead of straight vector lines). srcdoc iframe pattern NOT ported as code idiom — validates Sprint 3 sandbox approach is correct (same allow-scripts only), srcdoc improvement deferred to Sprint 6+ if CSP issues arise. v3.23 (2026-06-20 night, Sprint 6 — kgolid absorption begins) extends idiom registry with 2 more patterns from Kjetil Midtgarden Golid (kgolid): (9) chromotome palettes — ~23 hand-curated multi-color palettes (3-7 colors + bg + stroke) from hilda + jung + ranganath + kovecses sub-collections; major upgrade candidate for Atlas branding-palettes.js currently shipping only 5 simple 2-color presets; enables KPI cards each in different palette color + cultural/aesthetic diversity (vintage children's book / saturated animal / Indian textile / mid-century modern); MIT (c) kgolid; (10) Marching Squares (kgolid/topographic) — classic contour extraction from 2D scalar grid via 16-case dispatch, outputs line segments OR filled polygons per threshold; pure-JS no P5 dependency; includes buildNoiseGrid convenience; use for heatmap/density/topology visualization or decorative background pairing with chromotome (each contour level = palette color). No new worked examples in v3.23 (these idioms are infrastructure-level, Sprint 7+ may add worked examples once L3 validates LLM picks them up). v3.24 (2026-06-21 early morning, Sprint 7 — kgolid p5ycho apparatus deep dive) adds (11) generateApparatusGrid + drawApparatusGrid — kgolid's flagship algorithm from apparatus-generator (npm-style) + 4 evolution stages in p5ycho/apparat[1-4]; 9-block state machine + 3 probabilities + 4 color modes + H/V symmetry + ellipse boundary mask produces "robot anatomy / blueprint of a fictional machine" compositional aesthetic; Atlas engine ALSO has full port at sdf-js/src/ca/ca.js with SDF region generalization (engine layer, not iframe-accessible — this idiom file is the standalone inline-able variant for LLM); MIT (c) kgolid. 1 NEW worked example: "Engineering Org Apparatus" — 8 teams / 3 product lines content rendered as ~14×24 grid composition with grouped colored "rooms" sharing walls inside an ellipse boundary. Use this pattern when content describes COMPOSITIONAL structure (teams / components / modules / sub-systems / scheduled blocks) where the composition itself IS the message. Pairs perfectly with chromotome multi-color palettes. Sprint 7 also produced p5ycho survey (in README): 3 Sprint 8+ candidates (colonization DLA / lindenmayer L-system / weave multi-layer flow-field) + 5 deferred + ~50 skipped (visual demos with limited algorithmic reuse). v3.25 (2026-06-21 Sprint 8 — kgolid p5ycho trio CLOSE absorption) ports the 3 Sprint 7-deferred candidates, completing kgolid p5ycho absorption: (12) spaceColonization — organic branching tree growth via source attraction + pruning (process flow trees / dependency graphs / neural diagrams / "reach/expansion" visualizations), different from packCirclesInSDF (packs vs branches), static port for noLoop iframe (run all iterations in one call), deterministic via seed; (13) lSystemSegments + LSYSTEM_PRESETS — Lindenmayer L-system string-rewriting fractal plants via turtle graphics, 4 classic presets from Moussa's defaults (balanced/asymmetric/symmetric/wide_canopy), use for branding ornament / "growth" content metaphor / section dividers / background filler trees; (14) weaveFlowDashes — multi-layer Perlin-noise flow field as SHORT directional dashes (sister to moussa-perlin-flow-field which traces LONG streamlines; pick weave for dense dash texture, moussa for streamlines), per-cell N-sample gradient probe + N independent palette layers, deterministic, pairs with chromotome palettes for layered color woven look. **kgolid absorption COMPLETE**: 6 idioms total from kgolid sources (chromotome + marching-squares + apparatus + colonization + lindenmayer + weave). No new worked examples in v3.25 (these are infrastructure-level alternatives to existing idioms; next sprint should pause for L3 verification rather than continue accumulating). v3.26 (Sprint 11 — quality fixes from Sprint 10 L3 findings): no new idioms, no new worked examples. 2 prompt sections added: (1) JSON output discipline rule under p5-sketch subject type — explicit escape rules (\\\\, \\\", \\n) for args.code string field after Sprint 10 found ~10% of large-inline-code variants emitted invalid JSON; (2) "EXPECTED OUTPUT ANCHOR" block added under each of 5 worked examples — describes ✅ what good output looks like + ❌ common failure modes (black canvas, empty sketch, missing arrows, etc.) so LLM can self-check before emitting. Plus code change in src/compositor-api.js: callLiftLLM now has retry-with-exponential-backoff for transient errors (3 attempts, 1s/2s/4s, honors Retry-After header on 429, retries network failures + 5xx + 429, fails fast on other 4xx). Sprint 10 measured 27% transient error rate; v3.26 expected to cut to <10%. v3.27 (Sprint 12 — typography polish, "approach Napkin visual quality" per user direction 2026-06-21 overnight autonomous work): iframe sandbox now preloads Inter (sans-serif, weights 400/500/600/700/900) + IBM Plex Mono (mono, weights 400/500/700) via Google Fonts link. Sketches use textFont('Inter') / 'IBM Plex Mono' instead of system 'sans-serif' default. New "Typography" prompt section with hierarchy guidance (hero 56-96px / title 24-32 / label 14-18 / body 11-14) + drawingContext.font escape hatch for finer weight control. Replaces v3.20-v3.25 prompt default of textFont('sans-serif'). Updates p5-sketch subject-type spec to point to typography section. NEVER 'Arial'/'Helvetica' rule added. Visual upgrade: amateur system sans → polished modern UI typeface (closes the most visible Napkin/Beautiful.ai gap). v3.28 (Sprint 13 — iconography, second-most-visible Napkin gap): NEW idiom file atlas-icon-library.js with 24 curated SVG path icons (Heroicons + Tabler MIT-licensed) in 5 groups (people/charts/action/object/annotation). Inline-able as drawAtlasIcon(name, x, y, size, color). NEW "Iconography" prompt section enumerates all 24 names with affordance mapping + usage example (KPI card icon + label). LLM should add at least 1 icon per major content unit; Atlas P5 sketches without icons look sparse vs Napkin. 140/140 smoke tests pass (atlas-icon-library + 8 assertions). v3.29 (2026-06-21 — chart/diagram atom catalog) wires the full presentation-atom vocabulary into the archetype-first flow (new "Step 1b" section right after Step 3). The lift LLM previously knew almost no chart atoms (pyramid/bar mentioned once each; pie/column/line/kpi-card/org-chart/timeline/etc. absent) so it hand-rolled chart structure from raw primitives. Step 1b maps each Step-1 archetype to its preferred composite atom (sequence→progression/flow-chart/timeline; list→bullet-list/agenda-list; compare→matrix-grid/venn; hierarchy→org-chart/tree-diagram/pyramid/layer-stack; relation→relationship-graph/mindmap/radial-spoke/fishbone) + a data-chart table (bar/column/line/pie/funnel/waterfall/scatter/gantt/kpi-card/traffic-light) + shapes list, each with key args + trigger keywords (EN+中文). Directive: PREFER the one-subject atom over hand-rolling (saves 5-20 subjects, auto-framed); presentation atoms co-exist with v3.0/v3.2 scene atoms (pick by chart-slide vs pictorial-scene). Vocabulary-exposure only — NO new worked examples (per "more examples dilute adoption" lesson from v3.14/v3.15). Covers the 14 Wave A/B/C atoms shipped in #46 + the 6 pre-existing chart atoms. v3.30 (Sprint 14a — atoms-2d bridge) adds Priority 0 section to MODE_2D_ADDENDUM teaching LLM the 24 atoms-2d types (kpi-card / bar / column / line / pie / funnel / waterfall / gantt / sphere-fill / flow-chart / tree-diagram / org-chart / mindmap / relationship-graph / timeline / pyramid / progression / matrix-grid / shape / cube-grid / gear-cluster / puzzle-pieces / icon-badge / cover) with arg signatures. atoms-2d are Atlas-authored Canvas2D atoms rendering pseudo-3D PowerPoint-feel via main-page canvas (NOT iframe). Priority 0 = highest preference for business viz patterns (KPI hero / chart / diagram / pyramid / SWOT etc); fall back to SDF/P5 when content doesn't fit an atom. visual-panel.js routes atom-2d variants to atoms-2d/renderer; pipeline.js validates all-or-none constraint mirroring p5-sketch. v3.31 (Sprint 14b — finance preset library) adds Step 4.5 to MODE_2D_ADDENDUM: 5 composition templates (F1 KPI hero / F2 quarterly trajectory column / F3 waterfall bridge / F4 market share pie/donut / F5 line trend with annotations). Each preset: trigger phrases + atom-2d emit shape + cheatsheet decision row. Single-subject all-or-none constraint preserved (one preset per visual; multi-preset paragraphs split into separate visuals). v3.32 (2026-06-22 — Sprint 15c) extends MODE_2D_ADDENDUM with Step 5 listing 8 icon categories from the Phosphor bake (business / finance / tech / medical / hrm / social / signs / calendar — ~150 icons each, ~1100 unique). icon-badge atom now resolves names from hand-coded fast-path (24, v3.31 preserved) OR fallback baked library. LLM gains domain-appropriate icon vocabulary without needing to enumerate names manually; output icon names by kebab-case Phosphor convention. v3.33 (Sprint 15a — chart atom catalog expansion) adds 7 new chart atoms covering PresentationLoad chart category gap: bubble / histogram / break-even / stacked-area / seven-s-model / multiple-arrows / nine-field-matrix. Plus 3 extensions: matrix-grid gains quadrantAxes + bubbles for BCG; scatter gains regressionLine; bar gains targetLine. atoms-2d registry now 51 entries. v3.34 (Sprint 15b — infographic idiom atoms) adds 10 new atoms: circle-image-hub-spoke / magazine-column-grid / dashboard-multi-kpi-composite / device-mockup-frame / device-mockup-row / isotype-people-grid / isotype-prop-row / isotype-stat-comparison / infinity-loop-flow / kpi-water-drop. atoms-2d registry now 61 entries. v3.35 (2026-06-24, 3D end) teaches the shipped sphere-fill gauge (#134): `sphere-fill-3d` moves from the decorative Shapes list to the Data-charts table (it's a DATA atom — capacity / fill-level %), with `levels[]` + the requirement to set `material.kind:"fill"` + a liquid hue/sat/value. Plus carves a waterline exception into the "decorations are 2D-only" anti-pattern — a waterline on a fill/capacity gauge IS the data read, not decoration. Vocabulary-exposure only — NO worked example (per "more examples dilute" lesson). Closes the gap where #134's gauge shipped in the engine but the lift LLM never emitted `kind:"fill"`. v3.36 (2026-06-24, 3D end) teaches the per-sphere `colors[]` (each entry mirrors material — preset name or {hue,sat,value}) + `radii[]` args on `sphere-fill-3d`, so a multi-colour / multi-size fill-level set (e.g. the PresentationLoad D0961 cover: green 20% small / red 40% mid / blue 80% large) is ONE subject instead of N hand-split single-sphere subjects. Vocabulary-exposure only — no worked example. v3.37 (2026-06-24, 3D end) adds a "Text routing" section codifying the locked two-text-systems policy: data labels bound to geometry → in-scene SDF (chart `args.labels`; the fill gauge auto-labels its % from levels), but title / section-header / body / narration text → `scene.overlay[]` (flat DOM blocks anchored to a 3D point and projected to the screen, role ∈ title/head/body/kpi) — NOT 3D SDF glyphs. Stops the LLM from rendering descriptive copy as slow, ugly SDF text on text-heavy chart slides (p3/p7/p11/p15 DESCRIPTION + body). v3.38 (2026-06-25, 3D end) REFINES the policy after seeing the SDF font in production: the hand-built monoline SDF glyphs are too thin / not beautiful, so ALMOST ALL slide text now goes to the DOM overlay (real font). Adds a chip-less `value` overlay role for data readouts (gauge %, KPI number) — white bold, font auto-scaled from the object's `radius`, drawn directly on the object. The fill gauge NO LONGER auto-emits SDF % (expandChartLabels default-label removed); emit one overlay `value` block per sphere instead. SDF text (`text-3d-pipe` / `args.labels`) is now reserved for the narrow "engraved in/on geometry" case (curved-surface / occluded / surface-flush). Value readouts moved from SDF (#146) to overlay — much prettier.
---
...
# Step 1b: Reach for a chart / diagram atom (don't hand-roll the structure) ⭐⭐⭐

Atlas ships **composite chart & diagram atoms** — single-subject emits that build the whole structural pattern (nodes + edges + arrangement) for you. When the slide is a chart or diagram, **emit ONE atom instead of hand-rolling N primitives**. The atom IS the archetype realization from Step 1, auto-framed by the studio renderer (do NOT hand-tune camera distance to fit).

Emit shape: `{ "type": "<atom>", "args": {...}, "transform": { "translate": [0, <y>, 0] }, "material": "silver" }` — one atom = one `subjects[]` entry.

### Archetype → preferred atom

| Archetype | Preferred atom(s) | Key args | Trigger keywords |
|---|---|---|---|
| `sequence` | `progression-3d` · `flow-chart-3d` · `timeline-3d` | steps/stepRise; steps/gap; count/alternate | roadmap, process, pipeline, stages, growth, 流程, 路线图, 时间线 |
| `list` | `bullet-list-3d` · `agenda-list-3d` | items | agenda, key points, checklist, takeaways, 议程, 清单, 要点 |
| `compare` | `matrix-grid-3d` · `venn-3d` | rows/cols; sets/overlap | SWOT, 2×2, BCG, matrix, overlap, intersection, 矩阵, 对比, 韦恩图 |
| `hierarchy` | `org-chart-3d` · `tree-diagram-3d` · `pyramid-3d` · `layer-stack-3d` | levels/branching; levels; layers | org chart, reporting lines, taxonomy, decision tree, tech stack, OSI, 组织架构, 层级, 金字塔 |
| `relation` | `relationship-graph-3d` · `mindmap-3d` · `radial-spoke-3d` · `fishbone-3d` | count/edges; branches/leavesPerBranch; spokes; ribs | network, mind map, dependency, root cause, 关系图, 思维导图, 鱼骨图 |

### Data charts (the slide's focal object, or under `kpi-hero` for one highlighted chart)

| Atom | Use | Key args |
|---|---|---|
| `bar-3d` / `column-3d` | value bars (vertical / horizontal) | `values[]` |
| `line-3d` | trend / series | `points[]` |
| `pie-3d` | proportion / share | `values[]` |
| `funnel-3d` | sales / conversion stages | `stages` |
| `waterfall-3d` | cumulative gains/losses (bridge / P&L) | `count` or `deltas[]` |
| `scatter-3d` | correlation / XY distribution | `count`, `spread` |
| `gantt-3d` | schedule / project plan | `tasks` or `segments[]` |
| `kpi-card-3d` | single metric card | metric metadata |
| `traffic-light-3d` | RAG / status / go-no-go | `lights` |
| `sphere-fill-3d` | capacity / fill-level gauges (% full, liquid below a waterline) | `levels[]` (0–1); **set `material.kind:"fill"` + liquid `hue`/`sat`/`value`**; for multi-colour/size sets add per-sphere `colors[]` (each mirrors material) + `radii[]` — one subject, no need to split |

### Shapes (decorative / iconic objects, not data)

`cube-3d` (cluster/grid/tower) · `sphere-network-3d` / `sphere-tree-3d` / `sphere-segmented-3d` · `circle-frame-3d` / `-stack-3d` / `-segmented-3d` / `-loop-3d` · `arrow-3d` · `diamond-3d` · `gear-3d` · `cube-segmented-3d` · `puzzle-piece-3d`.

**Rules**:
- **PREFER the atom** over hand-rolling its structure from primitives — it saves 5–20 subjects and is auto-framed + internally consistent. Hand-roll only when no atom matches.
- These are PRESENTATION atoms. The v3.0/v3.2 **scene atoms** (buildings / nature / vehicles) still apply when lifting a 2D *pictorial world* rather than a chart slide. Both vocabularies co-exist — pick by whether the slide is a chart/diagram (use these) or a scene (use scene atoms).
- One chart atom per slide — it already embodies the whole archetype. Add only a `text-3d-pipe` title above it and minimal context; this also satisfies Step 3 (the atom's structure + title keep units legible).

# Text routing: ALL slide text → overlay; SDF only when engraved in geometry ⭐⭐⭐

**Almost all slide text is DOM overlay** (real font — crisp, weighted, pretty), projected from a 3D anchor so it tracks the camera. SDF glyphs are a hand-built monoline grotesk: thin and ugly — use them ONLY when text must be physically part of the object (engraved on a surface, wrapped on a curve, occluded by closer geometry). For a fill gauge / chart you essentially never need that.

`scene.overlay` = array of `{ text, anchor:[x,y,z], role, align? }`, `role` ∈ `title` / `head` / `body` / `kpi` / **`value`**. Each block sits on a dark frosted chip — EXCEPT `value`, which is chip-less white bold drawn directly on its object (gauge %, KPI number), font auto-scaled from the object's radius (pass `radius`).

- **Value readouts** (gauge `%`, KPI number) → overlay `value`, anchored at the object's centre, with its `radius`. The fill gauge does NOT auto-label — emit one `value` block per sphere.
- **Title / section headers (DESCRIPTION 1/2/3) / body / narration** → overlay `title` / `head` / `body`, anchored at the world point they label (e.g. above the sphere they describe).
- **SDF text** (`text-3d-pipe` / `args.labels`) → only when engraved in/on geometry. Default to overlay.
...
text-3d-pipe:     { "text": string, "pipeRadius"?: number,
                  ...
                  // Supports digits 0-9 + symbols (% . - + $ space) +
                  // uppercase A-Z (Wave 2 shipped 2026-06-19). Lowercase
                  // and punctuation pending Wave 3-4 — those silently drop.
...
### Community 2D ports (Track 4 batch — 2026-05-27, gbms MIT/IQ-credited)

These are FIRST-CLASS 2D primitive types — emit one subject + use `extrude`
or `revolve` to lift to 3D. All ported from Games-by-Mason/gbms.

```
cut-disk:      { "radius": number, "cut": number /* y-level of chord cut */ }
                  // D-shape: disk with top cut off by horizontal chord.
                  // cut=0 → exact lower half-disk; cut<0 → more than half;
                  // cut>0 → less than half. Stamps, lens cross, half-moon.

pentagon:      { "radius": number /* apothem */ }
                  // Regular 5-sided polygon, point-up. radius=apothem
                  // (center to mid-edge); outer vertex at radius*1.236.
                  // Badges, US-yield-sign, pentagonal icons.

octogon:       { "radius": number /* apothem */ }
                  // Regular 8-sided polygon, flat-top. Stop signs, octagonal
                  // frames. Note kebab spelling "octogon" matches gbms.

hexagram:      { "radius": number /* fold-line distance */ }
                  // 6-pointed star (Star of David). Outer tips reach
                  // approximately radius*2 from center. Jewish iconography,
                  // geometric ornament, occult symbol.

chamfer-box:   { "dims": [hx, hy] /* half-extents */, "chamfer": number }
                  // Rectangle with 45° cut corners (NOT circular like
                  // rounded_rectangle). Industrial / hardware / mechanical:
                  // gaskets, washers, electronic case cutouts, dog tags.

parabola:      { "k": number /* y = k*x² */ }
                  // The CURVE y = k*x², not a closed shape. Useful as a
                  // .source for `extrude` to make: arches (extruded
                  // parabola), dome silhouettes, lens cross-sections,
                  // fountain trajectories. k=1 → broad; k=4 → tight.
```

**Usage example** (pentagon extruded into a 3D plinth):
```json
{
  "id": "stop-sign-back",
  "type": "extrude",
  "args": { "depth": 0.1 },
  "source": { "type": "octogon", "args": { "radius": 0.5 } }
}
```
...
| **音乐会 / concert / 演唱会** (v3.14) | **audience × 30-60** (silhouettes facing stage), **stage lamps × 4-8** (array along stage top), **speakers × 2** (mirror flanking stage), **stage backdrop wall** (hand-built), 远处 cloud-puff 烟雾 | audience via `scatter` on rectXZ in front of stage; stage lamps via `array` along stage front edge axis=x; speakers via `mirror` plane=yz; stage wall hand-built; consider 1 `flame` or `god-rays` volume for stage lighting |
| **球场 / stadium / 体育场** (v3.14) | **观众席 tiered seats × N** (stacked array tiers), **scoreboard** (hand-built), **球员 / players × 4-10** scattered on field, **flag-on-pole × 2-3** (corners), **stadium lamps × 6-8** ring | tiered seats via **`array` × 2 (one per axis)** + **`mirror` plane=yz** for both sides; players via `scatter` on rectXZ; corner flags hand-placed; lamps via `array` around perimeter |
| **集市 / market / 夜市 / 摊位** (v3.14) | **stalls × 8-20** (small box+roof units), **shoppers × 5-15** (figure silhouettes scattered), **strung lights** (rep along y), **唐人街/拱门/牌坊** if Chinese market | stalls via `scatter` on rectXZ (size 30×30+); shoppers via `scatter` densely; strung lights via `array` along z; central gate hand-built. Consider warm `glow-warm` material kind on lights |
| **峡谷 / canyon / 大峡谷** (v3.14) | **terrain-canyon** as hero (already an Atlas primitive), **远峰 × 2-4** (cone + rock-boulder hand-placed), **birds × 3-5** soaring high, **river segment** if 2D shows water | use `terrain-canyon` primitive for the canyon itself; far peaks hand-placed; birds via `scatter` in box3 high above; river via `extrude` of a polygon path. Pair with cinematic (postFx + DoF + fog) — canyons are inherently cinematic |
| **海岸 / coastal / 海滩 / 礁石海岸** (v3.14) | **rock-boulder × 6-12** scattered along shore, **waves** (auto), **sailboat-small × 1-3** in water, **海鸥 × 4-8** overhead, **lighthouse × 0-1** if cliffs implied | rocks via `scatter` on rectXZ near shore; waves type already exists; sailboats via `scatter` on rectXZ in water; gulls via `scatter` in box3. If "礁石" mentioned, larger rock-boulders + more density |
...
## 🎁 Composite atoms (v3.13) — one-line shortcut for canonical scenes ⭐⭐⭐

When the prompt matches a **canonical scene pattern**, emit a single
composite-atom subject instead of writing out 25+ lines of peer subjects.
The compile-time expander unpacks the atom into the full constellation
(peer subjects + cinematic patches + sea + fog volume) for you.

### Available composite atoms (v3.13)

| `type` | Expands to | Use when |
|---|---|---|
...
{ "type": "carrier-strike-group",
  "args": {
    "escortCount": 4,         // N destroyer escorts (default 4)
    "escortSpread": [120,80], // rectXZ region (default [120,80])
    "separation": 25,         // min distance between escorts (default 25)
    "birdCount": 6,           // N gulls (default 6)
    "cloudCount": 3,          // N cloud-puffs (default 3)
    "sea": true,              // auto-add waves subject (default true)
    "cinematic": true         // auto-add postFx + aperture + fog (default true)
  }
}
```
...
{ "type": "airport-apron",
  "args": {
    "parkedPlaneCount": 4,
    "parkedPlaneSpacing": 8,
    "groundVehicleCount": 3,
    "lampCount": 12,
    "lampSpacing": 4,
    "cinematic": true
  }
...
Same prompt as Example 15 (`航母`). Same scene quality. **Token-cheaper.**

```json
{
  "v": 1,
  "subjects": [
    // ----- Hero carrier (unchanged from Example 15 — abbreviated here) -----
    {
      "id": "carrier",
      "type": "union",
      "children": [
    ...
    // ----- Composite atom: replaces Example 15's escort + gull + cloud + sea + cinematic -----
    {
      "id": "strike-group",
      "type": "carrier-strike-group"
      // No args needed — defaults are tuned for the canonical scene.
      // The expander at compile time will inject:
      //   - escort-destroyer (4 instances via scatter)
      //   - gull (6 instances via scatter, box3 sky volume)
      //   - cloud (3 instances via scatter, distant)
      //   - sea (waves subject)
      //   - defaults.postFx with bloom + vignette + lens-flare
      //   - defaults.camera.aperture: 0.6, focalDistance: 55
      //   - volumes[]: 1 sea-fog volume
    }
  ],
  "ground": { "y": -1.2, "region": "seabed",
    "material": { "hue": 0.58, "sat": 0.70, "value": 0.25 } },
...
1. **One line replaces ~25 lines of Example 15** — the atom expander runs
   server-side (in `compile()`), so you don't pay token cost for the
   peer-subject specs.
2. **Atom defaults are tuned for the canonical scene** — escortCount 4,
   birdCount 6, cloudCount 3, scatter region [120, 80]. These match what
   Example 15 emits byte-precision. Override only when scene calls for
   it (e.g. tiny demo scene: `escortCount: 2`).
3. **Hero subject stays hand-authored** — atom does NOT replace your
   carrier (or airport tower, or pier). It supplies the surrounding
   peers. You retain full control over the focal point.
4. **Atom cinematic patches MERGE with explicit defaults** — your
   `defaults.camera.yaw/pitch/distance` are preserved; the atom only
   ADDS aperture + focalDistance + postFx + fog volume. Author overrides
   atom's aperture by setting `defaults.camera.aperture` explicitly.
5. **Atom IS the v3.11 Scene-completion table, baked-in** — same byte-
   precision spec, just a function call. Use the atom when prompt is
   canonical; use Example 15's hand-emit when prompt has identifying
...
**v3.13 behavior (no leisure trigger in cheatsheet)**: LLM emits 1-2 stage
parts + maybe 1 speaker. Result: empty stage at a non-event. Mismatches
user mental model of "concert" = audience + lights + atmosphere.

**v3.14 behavior (REQUIRED for leisure trigger)**:

```json
{
  "v": 1,
  "subjects": [
    // ----- Hero stage + backdrop (hand-built from 2D code) -----
    { "id": "stage-floor", "type": "box",
      "args": { "dims": [16, 0.6, 8] },
    ...
    // ----- PEER: speakers (bilateral pair flanking stage via mirror) -----
    {
      "id": "speaker",
      "type": "box",
    ...
    // ----- PEER: stage lamps (array along front edge of stage) -----
    {
      "id": "stage-lamp",
      "type": "union",
      "children": [
      ...
      "variants": [{
        "op": "array", "count": 6, "axis": "x", "spacing": 2.6, "origin": "center"
      }]
    },

    // ----- PEER: audience (30 figures scattered in front of stage) -----
    {
      "id": "audience-figure",
      "type": "capsule",
      ...
      "variants": [{
        "op": "scatter", "count": 30,
        "region": { "type": "rectXZ", "center": [0, 0, 12], "size": [20, 12] },
        "separation": 1.0,
        "heading": "aligned",
        "scale":     { "jitter": 0.08 },
  ...
  // ----- Cinematic atmosphere (REQUIRED — concerts are inherently cinematic) -----
  "volumes": [
    { "id": "stage-haze",
      "kind": "fog",
      "center": [0, 3, 2],
  ...
  "ground": { "y": 0, "region": "concert-floor",
    "material": { "hue": 0.10, "sat": 0.05, "value": 0.18 } },
  "defaults": {
    "camera": { "yaw": 0.0, "pitch": 0.18, "distance": 24, "focal": 1.6,
                "targetX": 0, "targetY": 2, "targetZ": 6,
                "aperture": 0.7, "focalDistance": 15 },
    "light": { "azimuth": 0.0, "altitude": 0.35, "distance": 30, "intensity": 1.6 },
    "postFx": { "exposure": 1.2, "bloomMix": 0.45, "vignetteStrength": 0.5,
                "lensFlareStrength": 0.25 }
  }
}
```
...
1. **Audience via `scatter`** — 30 capsules (figure silhouettes) on rectXZ
   in front of stage. `separation: 1.0` keeps figures from overlapping
   while letting them be densely packed. `heading: "aligned"` because
   crowd faces stage.
2. **Stage lamps via `array`** — `count: 6, axis: 'x', spacing: 2.6` over
   stage front edge. `emissive` material kind for the bulb (warm color
   + glow=0.9 lets renderer bloom contribute).
3. **Speakers via `mirror`** — author one speaker at `translate: [8, ...]`,
   `mirror: yz` plane gives the matching pair at `[-8, ...]`. Tower
   speakers flanking stage is the canonical bilateral.
4. **Stage haze fog volume** — concerts are intrinsically cinematic; the
   warm-tinted `fog` volume catches stage lights and gives that "stage
   atmosphere" feel. `density: 0.12` is denser than open-sea fog (0.06)
   because concert haze is intentional.
5. **Camera high `bloomMix` + `aperture`** — bloom 0.45 (vs cathedral 0.25)
   amplifies stage emissive bulbs; aperture 0.7 with focalDistance 15
   keeps audience sharp + back of stage soft.
6. **Counter-modifier**: prompt like "肖邦钢琴独奏会" (Chopin piano solo)
   is intimate, NOT mass-concert. Use small audience count (8-15) +
   subdued bloom + no haze. Bare "音乐会" triggers MAX completion;
   modified versions scale back.
...
### Example 18c: Cover slide with letter title (v3.17) ⭐

**Prompt context**: a cover slide for a vision/keynote presentation — a single hero title like "VISION 2030" or "ATLAS" floating in scene center.

**Output**:

```json
{
  "id": "cover-vision-2030",
  "type": "text-3d-pipe",
  "args": { "text": "VISION 2030", "height": 1.2, "pipeRadius": 0.14, "align": "center" },
  "transform": { "translate": [0, 1.5, 0] },
  "material": "polished-chrome"
}
```

**Why**: Wave 2 added uppercase A-Z so titles render as actual SDF text. `pipeRadius: 0.14` (mid-thick) gives keynote-stage weight without being chunky. `polished-chrome` for sculptural shine. Camera should dolly in slowly (cameraSequence with single shot) for cinematic impact.

**Trap**: lowercase letters (e.g. "Vision 2030") would silently drop the lowercase chars in Wave 2 — render shows only "V 2030". Wait for Wave 3 (a-z) before mixed-case lifts.
...
- ❌ DON'T copy every 2D layer to 3D. Decorations (markings, outlines) are
  2D-only. Skip them. **Exception**: a waterline on a fill/capacity gauge is
  the data read, not decoration — emit `sphere-fill-3d` with
  `material.kind:"fill"` (the shader draws the waterline), don't skip it.
