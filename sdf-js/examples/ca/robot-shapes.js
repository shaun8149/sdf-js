...
// =============================================================================
export const ROBOT_CLASSIC = sdf
  .union(
    // 头（小环形）
    sdf.circle(0.12).difference(sdf.circle(0.03)).translate([0, -0.77]),
    // 脖子（细长）
    sdf.rectangle([0.06, 0.1]).translate([0, -0.61]),
    // 胸 = 大矩形 - 底部正方形（U 形）
    sdf
      .rectangle([0.5, 0.32])
      .translate([0, -0.38])
    ...
    sdf.rectangle([0.08, 0.4]).translate([-0.055, -0.24]),
    sdf.rectangle([0.08, 0.4]).translate([0.055, -0.24]),
    // 小腿（贴大腿底，向外阶梯式变粗 = calf）
    sdf.rectangle([0.13, 0.36]).translate([-0.08, 0.14]),
    sdf.rectangle([0.13, 0.36]).translate([0.08, 0.14]),
    // 脚（与小腿 0.04 间隙，再向外阶梯一点）
    sdf.rectangle([0.16, 0.06]).translate([-0.1, 0.39]),
    sdf.rectangle([0.16, 0.06]).translate([0.1, 0.39]),
  )
  .scale(1.2)
  .translate([0, 0.1]);
