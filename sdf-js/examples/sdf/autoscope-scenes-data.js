...
function adhocTriPrism(halfWidth, halfLength, translate) {
  return {
    id: adhocId('tri_prism'),
    type: 'tri_prism',
    args: { halfWidth, halfLength },
    transform: { translate },
  };
}
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (rng.random_bool(0.5)) {
      ...
      parts.push(
        autoscopePyramid(
          pos,
          [rng.random_num(10, 20), rng.random_num(1, 10), rng.random_num(10, 20)],
          rng.random_angle(),
        ),
      );
    }
...
function inhabitantsParts(rng, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (t === 0) parts.push(person1(pos, h, movement));
      ...
      parts.push(
        animal1(pos, rng.random_num(0.25, 1.25), Math.abs(movement[0]), rng.random_angle()),
      );
    else
      parts.push(animal2(pos, rng.random_num(0.25, 1), Math.abs(movement[0]), rng.random_angle()));
  }
  return parts;
}
...
function birdsParts(rng, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (t === 0) parts.push(bird1(pos, rng.random_num(0.05, 0.4), rot, speed));
      ...
      parts.push(bird3([pos[0], rng.random_num(6, 13), pos[2]], 0.2, rot, rng.random_num(2, 12)));
...
const SHADOW_MODES = [
  ...
  'darken',
  'darken',
  'darken', // 50% — neutral, atmospheric
  'hueRotate180',
  'hueRotate180', // 25% — complementary, dramatic
...
function makeDefaults(rng, opts = {}) {
  ...
  const animMode = rng.random_choice([
    'static',
    'yaw',
    'yaw',
    'dolly',
  ...
  if (animMode === 'lightSwing' || rng.random_bool(0.15)) {
    ...
    lightAnimation.push({
      channel: 'azimuth',
      value: sumT(rng.random_num(0.3, 0.7), sinT(amp, 0.08, 0)),
    });
  }
...
export function scene0_city_data(rng, opts = {}) {
  ...
  for (let i = 0; i < buildings; i++) {
    ...
    if (t === 0)
      subjects.push(boxE(pos, [rng.random_num(2, 6), rng.random_num(4, 20), rng.random_num(2, 6)]));
    else if (t === 1)
      subjects.push(
        houseS(
          pos,
          [3 * rng.random_int(1, 2), rng.random_num(5, 20), 3 * rng.random_int(1, 2)],
          rng.random_angle(),
        ),
      );
    else if (t === 2)
      ...
      subjects.push(
        arch2(pos, [rng.random_num(4, 8), rng.random_num(2, 16), rng.random_num(2, 20)], 0),
      );
    else
      subjects.push(
        arch(pos, [rng.random_num(4, 8), rng.random_num(2, 16), rng.random_num(2, 20)], 0),
      );
  }
...
export function scene1_sea_data(rng, opts = {}) {
  ...
  if (rng.random_bool(0.2)) {
    ...
    subjects.push(
      adhocCylinder(rng.random_num(0.1, 0.4), rng.random_num(5, 15), {
        rotate: [angle, 0, 0],
    ...
    subjects.push(
      autoscopeCone(
        [rng.random_num(-14, 14), -1, rng.random_num(5, 25)],
        rng.random_num(0.5, 2),
        rng.random_num(5, 15),
      ),
    );
    ...
    subjects.push(
      cylinderE(
        [rng.random_num(-10, 10), 0, rng.random_num(5, 20)],
        rng.random_num(7.5, 25),
        rng.random_num(1, 4),
      ),
    );
  }
  ...
  if (rng.random_bool(0.15)) {
    subjects.push(
      adhocSphere(rng.random_num(0.25, 0.75), [rng.random_num(-10, 10), 0.1, rng.random_num(0, 5)]),
    );
  }
...
export function scene2_forest_data(rng, opts = {}) {
  ...
  for (let i = 0; i < treeCount; i++) {
    ...
    if (t === 0) subjects.push(tree1(pos, rng.random_num(3, 10)));
      ...
      subjects.push(
        adhocSphere(tr, [
          rng.random_int(-12, 12),
          tr * rng.random_num(0.3, 0.5),
          rng.random_int(10, 40),
        ]),
      );
    } else if (t === 3) {
      subjects.push(
        cylinderE(
          [rng.random_int(-15, 15), 0, rng.random_int(-40, 40)],
          30,
          rng.random_num(0.15, 0.3),
        ),
      );
    } else if (t === 4) {
      subjects.push(
        tree3(
          [rng.random_int(-12, 12), 0, rng.random_int(10, 40)],
          rng.random_num(5, 12),
          rng.random_angle(),
        ),
      );
  ...
  if (rng.random_bool(0.33)) {
    if (rng.random_bool(0.5)) {
      subjects.push(
        houseS(
          [rng.random_num(-10, 10), 0, rng.random_num(-5, 30)],
          [rng.random_num(3, 5), 3, rng.random_num(3, 9)],
          rng.random_angle(),
        ),
      );
    } else {
      subjects.push(
        houseP(
          [rng.random_num(-10, 10), 0, rng.random_num(-5, 30)],
          [3 * rng.random_int(1, 4), rng.random_num(3, 6), 3 * rng.random_int(1, 3)],
          3,
          rng.random_angle(),
        ),
      );
...
export function scene3_village_data(rng, opts = {}) {
  ...
  for (let i = 0; i < buildings; i++) {
    ...
    if (t === 0)
      subjects.push(
        houseS(pos, [rng.random_num(3, 5), 3, rng.random_num(3, 20)], rng.random_angle()),
      );
    else if (t === 1)
      subjects.push(
        houseP(
      ...
      subjects.push(
        houseC(pos, [3 * rng.random_int(1, 4), rng.random_num(3, 6)], rng.random_num(3, 6)),
      );
  }
  ...
  if (rng.random_bool(0.1)) {
    subjects.push({
      ...boxE(
        [rng.random_num(-20, 20), 0, rng.random_num(-5, 40)],
        [3 * rng.random_int(2, 4), rng.random_num(6, 9), rng.random_int(1, 9)],
      ),
      transform: {
  ...
  if (rng.random_bool(0.2)) {
    subjects.push(
      cylinderE([rng.random_num(-20, 20), 0, rng.random_num(-5, 40)], rng.random_num(4, 20), 0.2),
    );
  }
  ...
  if (rng.random_bool(0.33)) {
    ...
    for (let i = 0; i < treeCount; i++) {
      ...
      if (t === 0) subjects.push(tree1(pos, rng.random_num(3, 10)));
        ...
        subjects.push(
          cylinderE(
            [rng.random_num(-15, 15), 0, rng.random_num(-40, 40)],
            30,
            rng.random_num(0.15, 0.3),
          ),
        );
...
export function scene4_axis_data(rng, opts = {}) {
  ...
  for (let i = 0; i < buildings; i++) {
    ...
    if (t === 0)
      ...
      subjects.push(
        arch2(
          [rng.random_num(-10, 10), 0, rng.random_num(-5, 15)],
          [rng.random_num(4, 8), rng.random_num(2, 16), rng.random_num(2, 20)],
          0,
        ),
      );
    else {
      ...
      if (repCount === 0) {
        subjects.push({
          ...archUnit,
          transform: { ...(archUnit.transform || {}), ...outerTransform },
        });
      } else {
        subjects.push({
          id: `axis-arch-rep-${i}`,
