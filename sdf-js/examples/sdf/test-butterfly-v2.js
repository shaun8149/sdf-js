...
const bodySegments = intersection(
  body,
  union(
    rectangle([0.1, 0.012], [0, 0.08]),
    rectangle([0.1, 0.012], [0, -0.02]),
    rectangle([0.1, 0.012], [0, -0.12]),
    rectangle([0.1, 0.012], [0, -0.22]),
  ),
);
...
const eyes = union(circle(0.012, [-0.025, 0.31]), circle(0.012, [0.025, 0.31]));
...
export function scene(ctx) {
  render.silhouette(
    ...
    [
      ...
      { sdf: dilate(bodyAndHead, 0.02), color: OUTLINE },
