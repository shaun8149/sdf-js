...
const leftTower = rectangle([0.55, 1.55], [-0.7, 0.025]);
const rightTower = rectangle([0.55, 1.55], [0.7, 0.025]);
const centerWall = rectangle([0.85, 1.1], [0, -0.2]);
...
const leftSpire = polygon([
  [-0.975, 0.8],
  [-0.425, 0.8],
  [-0.7, 1.12],
]);
const rightSpire = polygon([
  [0.425, 0.8],
  [0.975, 0.8],
  [0.7, 1.12],
]);

const stone = union(leftTower, rightTower, centerWall, centerGable, leftSpire, rightSpire);

const stringcourses = intersection(
  union(rectangle([2.5, 0.025], [0, -0.22]), rectangle([2.5, 0.025], [0, 0.32])),
  stone,
);

const portals = union(
  lancet(0, -0.75, 0.28, 0.52),
  lancet(-0.7, -0.75, 0.2, 0.38),
  lancet(0.7, -0.75, 0.2, 0.38),
);
...
const traceHub = circle(0.04, roseCenter);

const towerWinLow = union(lancet(-0.7, -0.1, 0.22, 0.36), lancet(0.7, -0.1, 0.22, 0.36));
const towerWinUp = union(
  lancet(-0.8, 0.4, 0.11, 0.28),
  lancet(-0.6, 0.4, 0.11, 0.28),
  lancet(0.6, 0.4, 0.11, 0.28),
  lancet(0.8, 0.4, 0.11, 0.28),
);
...
const leftCross = union(
  rectangle([0.022, 0.1], [-0.7, 1.17]),
  rectangle([0.06, 0.02], [-0.7, 1.18]),
);
...
const moon = circle(0.1, [0.32, 0.95]);
...
export function scene(ctx) {
  render.silhouette(
    ...
    [
      ...
      { sdf: dilate(portals, 0.02), color: OUTLINE },
      { sdf: portals, color: DOOR_C },

      { sdf: dilate(crosses, 0.01), color: OUTLINE },
